<script lang="ts">
  ...
  let projectPath = "demo-project";
  let projectMode = "preview";
  let manifest: any = null;
  let selectedShot = "shot-01";
  async function projectAction(command: string) { try { status = `${command}...`; manifest = await invoke("project_workflow", { command, project: projectPath, story: command === "create" ? storyText : null, shot: ["retry", "review"].includes(command) ? selectedShot : null, mode: ["run", "resume"].includes(command) ? projectMode : null }); status = `${command} 完成`; } catch (e) { status = `${command} 失败: ${e}`; } }
  ...
  async function generateImage() { if (!storyReady) { status = "请先生成故事"; tab = "story"; return; } try { const story = JSON.parse(storyInput); const result: any = await invoke("run_media_stage", { stage: "image", input: JSON.stringify({ scene: story.scenes?.[0], source_spans: story.scenes?.[0]?.source_spans ?? ["scene-1"] }) }); quality = result.quality_evaluation; stageOutput = JSON.stringify(result, null, 2); imageReady = true; status = "图片已生成"; tab = "home"; } catch (e) { status = `图片失败: ${e}`; } }
  async function generateVideo() { if (!imageReady) { status = "请先生成首帧/尾帧图片"; tab = "image"; return; } try { const result: any = await invoke("run_media_stage", { stage: "video", input: JSON.stringify({ first_frame_ref: "artifact://首帧", last_frame_ref: "artifact://尾帧", source_spans: ["scene-1"], scene: { summary: videoPrompt }, prompt: videoPrompt }) }); quality = result.quality_evaluation; stageOutput = JSON.stringify(result, null, 2); status = "视频已生成"; tab = "home"; } catch (e) { status = `视频失败: ${e}`; } }
...
<div class="shell"><aside class="sidebar"><div class="brand"><span class="brandmark">M</span><div><strong>Story Media</strong><small>ORCHESTRATOR</small></div></div><nav><button class:active={tab === "home"} on:click={() => tab = "home"}>⌂ 首页流水线</button><button class:active={tab === "story"} on:click={() => tab = "story"}>▣ 生成故事</button><button class:active={tab === "image"} on:click={() => tab = "image"}>◈ 生成图片</button><button class:active={tab === "video"} on:click={() => tab = "video"}>▶ 生成视频</button></nav><div class="sidebar-note"><strong>执行引擎</strong><small>Rust trusted runtime</small><small>模型无关 · 可控返工</small></div></aside><main>
  ...
  <aside class="right-panel"><section><h3>Artifacts</h3>{#if run}{#each run.stages as stage}{#if stage.artifact}<div class="artifact-row"><span>{stage.name}</span><code>{stage.artifact}</code></div>{/if}{/each}{:else if stageOutput}<div class="artifact-row"><span>当前阶段输出</span><code>已生成</code></div>{:else}<p class="muted">暂无 artifact</p>{/if}</section><section><h3>质量门禁</h3><div class="gate-row"><span>质量模型决策</span><b class:pass={quality?.decision === "passed"} class:warn={quality?.decision === "warning"} class:fail={quality?.decision === "failed"} class="ok">{qualityLabel()}</b></div><div class="gate-row"><span>检测原因</span><b class="ok">{quality?.reason ?? "等待 artifact"}</b></div>{#if quality?.failures}<div class="gate-row"><span>失败指标</span><b class="fail">{quality.failures.length}</b></div>{/if}</section><section><h3>运行日志</h3><p class="log">{status}</p>{#if run}<p class="log">run_id: {run.run_id}</p><p class="log">状态: {run.status}</p>{/if}</section></aside>
  ...
  {#if tab === "home"}
    <section class="card"><h2>持久化 Preview 项目</h2><div class="row"><label>项目目录<input bind:value={projectPath}></label><label>模式<select bind:value={projectMode}><option value="preview">Preview</option><option value="render">Render</option></select></label><label>镜头<input bind:value={selectedShot}></label></div><div class="actions"><button on:click={() => projectAction("create")}>创建</button><button class="primary" on:click={() => projectAction("run")}>生成 Preview</button><button on:click={() => projectAction("resume")}>恢复</button><button on:click={() => projectAction("retry")}>重试镜头</button><button on:click={() => projectAction("review")}>审核通过</button></div>{#if manifest}<pre>{JSON.stringify(manifest, null, 2)}</pre>{/if}</section>
    ...
    <section class="flow"><button type="button" class:done={stageState(0) === "succeeded"} class:active={stageState(0) === "running"} class="flowstage" on:click={() => tab = "story"}><span>01</span><h3>生成故事</h3><strong>{stageState(0)}</strong><small>输入故事梗概，输出 story-package</small></button><i>→</i><button type="button" class:done={stageState(1) === "succeeded"} class:active={stageState(1) === "running"} class="flowstage" on:click={() => tab = "image"}><span>02</span><h3>生成图</h3><strong>{stageState(1)}</strong><small>首帧 / 尾帧与角色连续性</small></button><i>→</i><button type="button" class:done={stageState(2) === "succeeded"} class:active={stageState(2) === "running"} class="flowstage" on:click={() => tab = "video"}><span>03</span><h3>生成视频</h3><strong>{stageState(2)}</strong><small>MiniMax H3 · 5 秒动作单元</small></button></section>
